...
fn main() {
    ...
    let find_data = |start_offset: usize, start_word: String, end_word: String| -> (usize, String) {
        let start = string[start_offset..].find(&start_word).unwrap();
        let end = string[start_offset + start + start_word.len()..].find(&end_word).unwrap();
    ...
    println!("{}", risc0_zkvm::guest::env::cycle_count());
}